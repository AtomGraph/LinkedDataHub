## [6.0.0]
Bootstrap 2 is gone, and with it the class vocabulary and the template modes application stylesheets were written against. The platform stylesheet moves from `xsl/bootstrap/2.3.2/layout.xsl` to `xsl/layout.xsl`, and every `bs2:` mode takes the name of the design-system component it renders. An `xsl:import` of the old path fails to compile, which at least says so; an override of a `bs2:` mode, or one that emits Bootstrap 2 markup, still compiles and still runs — the override never fires, the classes are backed by no stylesheet, and the page renders unstyled with no diagnostic. Hence the major version: the failure gives no signal of its own.
...
- The platform stylesheet is `xsl/layout.xsl`, and every `bs2:` mode takes its design-system component name — `bs2:Row` becomes `ldh:BlockRow`, `bs2:Header` `ac:BlockHeader`, `bs2:PropertyList` `ac:PropertyEditor`, `bs2:Form` `ac:ResourceForm`, `bs2:NavBar` `ac:Header`, `bs2:Lookup` `ldh:Combobox`, `ac:ModeList` `ac:ModeSwitcher`, `ldh:LeftSidebar` `ldh:DataspaceDrawer`. An `xsl:import` of `xsl/bootstrap/2.3.2/layout.xsl` fails to compile; an override of a `bs2:` mode compiles and never fires
- `ldt:base()` becomes `lapp:base()` and the `$ldt:base` parameter goes, so a stylesheet calling the function or reading the parameter no longer compiles. The Java `LDT` vocabulary, the `ldt#ontology` `Link` relation and the `ldt:` terms in `config/*.trig` are untouched
- One `fuseki` service holds every dataspace role as its own TDB2 dataset: `fuseki-admin` and `fuseki-end-user` leave `docker-compose.yml`, and `config/fuseki/config.ttl` declares an end-user and an admin dataset per dataspace, named after the origin with the deployment host dropped and the role appended (`end-user`, `admin`, `northwind-traders.demo.end-user`, …), each persisted under `fuseki/<dataset>/`. An existing deployment moves its two TDB2 directories under the new dataset names, points every `ldt:service` in `config/system.trig` at `http://fuseki:3030/<dataset>/`, and merges the two stores' heap and `mem_limit` into the one service. `varnish-admin` and `varnish-end-user` stay, both fronting `fuseki`
- Package stylesheets reach the client through a SEF composed per import set by a new `sef-compiler` service: a deployment with its own compose file adds the service, the platform's `depends_on` on it and the `./sef` bind mount at `/var/www/linkeddatahub/sef`, served under `/static/xsl/sef/`. `SEF_ROOT` and `SEF_COMPILER` are optional; without them packages compose server-side only, as in 5.10.0. `CLIENT_STYLESHEET` names the client stylesheet the SEF is composed from and defaults to the platform's `client.xsl`: a deployment whose page bootstraps its own client stylesheet sets it to that stylesheet's webapp path - it must import the platform's `client.xsl` directly, by an href that resolves from that path - and its layout prefers `ldh:client-stylesheet()` over its own SEF, or the site's rules vanish from a package dataspace on the first client-side navigation
- A package stylesheet is composed right after `hooks.xsl` rather than above every core module: it can specialise an open mode — `ldh:RowHook`, the tree node, and the property row, value cell, form control and annotation leaves in `imports/values.xsl` — and can no longer replace a sealed one, a typed rule or a global; a package rule matching `rdf:RDF` in `ac:Head` no longer replaces the page
- The ACL query fails closed on a typeless resource (see Security). The `oauth2-login` and `oauth2-authorize` grants move from the admin store to the end-user app's `namespace-ontology.trig.template` as `acl:accessTo` on the four login and callback URLs, so an existing deployment re-seeds or migrates those authorizations. A non-owner request for a URL that does not exist is answered 403 rather than 404, since nothing authorizes a typeless URL before the handler could report it missing
- `ldh:container` leaves the ontology, the `ldh:ViewConstructor` form and the view query: a view's Create button determines the container from the view's own solutions, and an ontology declaring `ldh:container` on a view loses only the triple
...
- `ldh put` and `ldh post` take an optional `FILE` argument and recognize the RDF syntax from its extension (`ldh put "$LDH_BASE" root.ttl`); `-t/--content-type` stays required when reading stdin, which carries no name to go by
- Every dataspace serves its own `/sitemap.xml`, generated at startup from the public read rules of its own admin service, so an origin lists only its own documents — what the sitemaps protocol requires of a sitemap at that location
- `/robots.txt` per dataspace: one that has a sitemap names it and allows crawling, and one that has none — an admin dataspace, or an end-user dataspace with nothing public — disallows everything
- A `RewriteValve` and `WEB-INF/rewrite.config` answer both paths from the file generated for the requested host, since a servlet mapping dispatches on path and never on `Host`. Neither is a document of the platform's own model, so nothing in the application knows they exist
- `http-tests/dataspaces/sitemap.sh` pins both paths answering per origin — a configured host from its own file, an unconfigured host not at all. No fixture of that stack grants public read to an end-user document, so the cross-dataspace assertion the two dataspaces exist for cannot run there yet, and the script says so rather than skipping it silently
- `ldh push` replays a directory into the document tree it maps to — the apps' `update-folder.sh` as a CLI command. An RDF file, in any syntax Jena tells from the extension, is `PUT` to the document its path spells with relative URIs resolved against that URL; `root.ttl` at the top is the target document itself; every other file is uploaded into its directory's document; a subdirectory recurses under the child document its sibling RDF file describes. `.ldhignore` applies gitignore-style to its whole subtree, `--dry-run` prints the plan without a request or a certificate, one written URL per line goes to stdout and the first failed request stops the run. The walk and the ignore rules are unit-tested, and `http-tests/push/` pushes a fixture tree and checks the mapping, the skips, the upload and the convergence
- `ldh create item` and `ldh create container` take `--primary-topic`, resolved against the created document (`'#this'` names a fragment, an absolute URI a resource described elsewhere), so a script can say what a document is about without a second `PATCH`. It writes the link only
- The page renders in whichever colour scheme the reader's browser asks for: `color-scheme: light dark` on the root, every token that differs written once as `light-dark()`, a `<meta name="color-scheme">` so a dark reader gets no white flash, and `data-theme` kept only as a forcing override. Chart tokens are registered with `@property` so `getComputedStyle` resolves them per scheme; CodeMirror, YASQE and the RDFa editor chrome take the tokens; the map stays light by decision
- A responsive axis: the shell (header, tab strip, action bar, footer, drawer) queries `@media` at 768 and 1024, content components query `@container` at 520 on the card, modal body, block and content aside; the action bar wraps into two tiers below 640, block corner affordances join the flow where the device cannot hover, and the page stops overflowing at phone width
- A document's markup asserts the graph the same URL serves: value cells carry `@content` where what a cell shows and what it asserts differ (a date renders formatted and asserts its lexical form), `ldh:DocumentMetadata` carries the document's own type, title, timestamps, owner, parent, topic and `rdf:_N` membership as `link`/`meta` carriers, an XHTML block says it is `rdf:value`, and `@about` on the root pins the default subject to the canonical URL. `http-tests/rdfa/` pins the round trip
- Package stylesheets reach the client: the platform composes a wrapper importing `client.xsl` and the imported packages, has the `sef-compiler` service compile it, and publishes the SEF under `/static/xsl/sef/<key>`, keyed on the stock SEF's digest and the sorted package URIs; the URL travels to the page as a `Link` header. Server-side composition is never withheld, so a package still takes effect on the next request and the client catches up after one compile
- `ldh:ContentColumn`, a declared slot in `ldh:ContentBody` that the platform wraps in `.ldh-content-aside` when a package fills it, and a tree component (`xsl/tree.xsl` in the shared trunk, `client/tree.xsl` for the handlers) the document tree is one consumer of. The taxonomy editor package fills the slot with a concept tree: rooted at the scheme, opened down to the concept being read, a polyhierarchical concept under each of its parents, rows linking into `ReadMode`. The SKOS ontology gains views for `skos:hasTopConcept` and `skos:inScheme`, an orphan-concepts view, a missing-`inScheme` constraint and `rdf:langString` constructors for the label properties
...
- Dropping an RDF file anywhere on a document imports it, and the page says where it lands: a fixed overlay raised on the first file `dragenter` takes the drop, every drag rule yields on a payload it does not own, and a successful import lands in `ReadMode`
- An HTTP error document tells the reader what the status means: the block header becomes an inline alert with the sentence `ac:http-error-key()` picks, keeping the description rows and the Request access button; the error builders move to Web-Client
- Every failed client request reports where it was made: `ldh:promise-failure` takes the host element and a headline at every promise, `ldh:RenderFailure` picks the shape by host (an error row in a list, an alert in a form's fieldset, a tag on a one-line host), a timed-out request reads as one that did not complete, and `alert()` remains only for a host that has left the page
- Keyboard operability: menus take the `role=menu` model (focus moves in on open, Arrow, Home and End rove, Escape restores the trigger), the RDFa annotation object switch answers the arrow keys on its radiogroup with a roving `tabindex`, and the pager's previous and next become buttons
...
- `ui-tests/`: a Playwright suite driving the running stack in an owner and an anonymous project — client certificates for both origins, a console, page-error, alert and 4xx collector that fails the provoking test, a hydration gate on Saxon-JS's listener binding, fixtures seeded with `ldh`, and a preflight that refuses a stale SEF. Runs in CI as its own workflow (`make ui-tests`)
- `http-tests/system/`: 19 auth-boundary scripts over `/settings`, `/sparql`, `/ns`, `/access` and `/clear` for the owner, an agent, a reader and an unauthenticated client, joined to `run.sh`, with `lapp:allowRead` and OAuth-login-reachable regression tests
- `http-tests/sparql-protocol/query/GET-sparql-xhtml.sh` asks the endpoint for a rendered result set, the first test to reach the XSLT writer on that path, and an import test declares a stylesheet-only package described on the instance and requires the settings `PATCH` and the first render to answer within 30 s

### Changed
- **BREAKING**: the bundled package is named for what it does rather than for the vocabulary it speaks: `https://packages.linkeddatahub.com/skos/#this` becomes `https://packages.linkeddatahub.com/editor/taxonomy/#this`, opening an `editor/` namespace for the editors that follow, and its stylesheet becomes `skos.xsl` — the package is the taxonomy editor, that module is the SKOS-specific part of it. An `ldh:import` naming the old URI resolves to nothing and renders the application without the package: re-declare it with the new one
...
- A view's parallax row becomes a closed-by-default `<details>` disclosure, and Create moves out of the view toolbar into the block header beside Copy; the toolbar states what the query shows (filters, sort, count, mode)
- The query block runs its stored query on render, results first, with the editor folded behind a toggle in the block head; YASQE mounts in the dark SPARQL pane and Run is the one primary action. The editor fills the code field on both axes, with a fifteen-line floor so an empty query field opens at a height a query gets written in
...
- XHTML prose blocks render without a header — the drag grip is a hover-surfaced strip on the card's edge and links, Copy and Edit a corner cluster; an `ldh:Object` block is a chrome-less carrier with a slot bar over the embedded card, so a block has one header. Instance timestamps leave the block chrome and render as ordinary statements
- A collection of resources renders as `ul`/`li` (view rows and cards are list items) and a read-mode property list rides `dl`/`dt`/`dd` — the markup is the representation the RDFa is carried on
...
- Paging or re-sorting a view no longer collapses the block: the pager lives in its own host beside the results region and the rows stay standing, dimmed, while the query runs. Class state on the client is one vocabulary — `contains-token()` to read, `ldh:set-token()` to write — and the client XSL uses `ixsl:` instructions where it called DOM methods
- The floating Edit pill retires; every block carries the edit pencil in its own control cluster, and click-anywhere-to-edit lands on that button. The object-position lookups and `sh.xsl` move down to Web-Client, and the SSR and CSR import chains align so a generic resolves identically in both
- The combobox cluster takes design-system names (`ldh:Combobox`, `ldh:ComboboxChip`, `ldh:ComboboxItem`, …), taking the last `graphity.org` namespace out of the codebase; `RDFPostMediaTypeInterceptor` moves down to Web-Client
- The last hard-coded English leaves the client: the RDFa editor's toolbar, block and format options, dialogs and annotation form, the media-type option groups, the 3D graph's panels and the block drag and delete strings read from `translations.rdf`. The vendored RDFa editor is a byte-identical copy of `RDFa-Editor/src` again, and LDH's integration is four parameters and one function
- The RDFa annotation dialog pins the statement's subject and predicate and tabs the rest: an object tab leading with a Text or Link switch (a literal or a resource, never both) and a subject tab for the `@about` and `@typeof` overrides
- Constraint violations decorate the affected fields with the inline message instead of stacking generic alerts
- Leaving an editor leaves it: an XHTML block clicked out of unedited restores its snapshot without a save, so no version is minted for a no-op, while edited content saves; a press outside a whole-resource edit form dismisses an untouched form and leaves an edited one to Save or Cancel; a press between blocks moves the caret instead of exiting
- A grid card no longer offers an edit pin: a card is one projection of a result row, and the whole resource with its authorization is one hop away
...
- The document tree's children query is a bounded `CONSTRUCT` of what a node renders — URI, label, type, whether the tree relation is present — paged by `ldh:tree-page-size` (default 1000, raisable by an app stylesheet) and always including the child leading to the document being opened
- The shell carries its landmark roles (`banner`, `main`, `contentinfo`), the tab strip and breadcrumbs are labelled navigations whose visibility derives from content, the address bar is a `role=search` with a `type=url` input that works without CSR, and forms drop the state classes that lied
- Constructor instantiation is one shared pipeline behind a dual `ldh:parse-query` — SPARQL.js in the browser, a `ParseQuery` Jena extension function on the server — so the server instantiates constructors too, and the sign-up form appears on a direct page load
- `ldh:is-local()`, `ldh:service-endpoint()`, `lapp:application-description()` and one shared `Link` header parser replace the copies written out at every call site; six SPARQL queries duplicated between `layout.xsl` and `client.xsl` are single-sourced; `sh:name` and `sh:description` labelling moves into LDH, where it outranks `dc:title` and `foaf:name`
...
- Web-Client dependency bumped to 6.0.0
...
- `$ldt:base`, `$ldt:ontology`, `$ac:httpHeaders`, `$ac:method`, `$Referer`, `$ac:googleMapsKey`, `$doc-types`, `$main-doc` and `$acl:Agent` — parameters written by the Java side and read by nothing, or declared and never populated; the `ldt:` prefix leaves the LDH stylesheets
- `ldh-bridge.css`, merged into `ldh.css`; the dead Bootstrap residue it carried (`.control-group`, `form .btn-primary`, `.faceted-nav`, the 979px responsive block) goes with it
...
### Fixed
- The sitemap covers every dataspace rather than the root one, and an origin is served its own. Generation ran once against the root services, and the one file it wrote was mapped to Tomcat's default servlet, which cannot vary by host: a deployment isolating its stores left every other dataspace without a sitemap, while one sharing them served each origin a sitemap of another origin's documents. The entrypoint now pairs each end-user app with the admin app of its dataspace and passes each query that dataspace's own base, writing one file per host that the rewrite resolves the request to
- The sitemap queries carry the service credentials, as every other store call in the entrypoint already did. Against a store that requires authentication they were answered with 401, and the platform started with no sitemap and said only that it could not generate one
...
- A drop-down flips to whichever side of its trigger has room, on both axes, and caps its height to the room on that side, so the action bar's left-zone menu no longer opens off the viewport edge and a long Create menu scrolls instead of running under the sticky chrome
...
- The context dataset's read/write race: `updateApp` mutated the shared Jena dataset in place while every request read it unsynchronized, so a `PATCH /settings` concurrent with any request could throw `ConcurrentModificationException` or serve a torn model; writes are copy-on-write behind a volatile swap and the file is written through a temp file and an atomic move. Also the expiring auth caches' check-then-get races (an intermittent login 500), `EMailListener` running SMTP on the common pool, `GraphVersioningService` leaking one commit-chain entry per branch, and `XsltExecutableFilter` compiling each stylesheet once per thread on a cold start
- A `PATCH /settings` adding an `ldh:import` returned 204 but the package ontology never joined the `/ns` closure: the rebuild read the request's pre-swap application snapshot, and now re-reads the app from the current dataspace model
- Importing a package described on this instance answered 504: resolving its description was an HTTP `GET` of a document this application serves, which ran the ontology filter, which resolved the package again, nested until nginx cut it. A local package's named graph is read from the store, and only foreign URIs are dereferenced
- A `DOCTYPE` in an application's `ac:stylesheet` silently disabled package composition for that application, with an `ERROR` in the log as the only trace; the server parses through the same entity-tolerant reader the client already used, and an external entity still expands to nothing
- The `/access` endpoint under-reported class-based grants once the ACL query failed closed: it injects the resource's types the way `AuthorizationFilter` does, and document creation by `PUT` authorizes against the parent container's types rather than the wildcard that used to let it through
- Asking the SPARQL endpoint for a document (`Accept: text/html`, what a browser sends) answered 500 since the document tabs: the tab panel and document body matched `rdf:RDF` only. Both match a result set too, and the client's re-fetch keeps `query`, `default-graph-uri` and `named-graph-uri`
- The drawer tree fetched an unbounded `DESCRIBE` of every child of a container on every page render, so large containers exceeded the response limit and the tree answered 502 on every page beneath them — 343 KB now where 20.5 MB was
- A facet on a large container never returned: the value query's label lookup walked an eight-alternative property path under `GRAPH ?labelGraph`, which Jena evaluates over every graph in the store (24.9 s on the demo's orders); binding the predicate with `VALUES` takes 0.055 s. The x on a parallax step chip did nothing and raised an error, and a chart axis bound to a URI-valued variable printed escaped anchor markup — only the Table chart keeps the anchor, the rest get the label
- Filtering a facet on a language-tagged value matched nothing, since the tag was dropped at every step of the chain; clearing a filter left "No results." over the rows in hand; the empty state's Clear filters button never rendered
- A chart or view created from a query block replaced the query block, because the row form was anchored inside the block's own row; it lands beside it now, and the Create buttons require the Properties layout, since in content the flow wrote a direct `rdf:_N` member by accident. A chart pane's default series excluded nothing, so any plotted type raised "All series on a given axis must be of the same data type"; the default is the number columns minus the category
...
- Reordering a block on a document with an embedded block `PATCH`ed the embedded document — through the proxy, for an external URI, answered 502 and reverted: the target is the dragged block's document, not the one under the pointer
- A paged view's grid mode escaped its block and painted over the footer; the body is uncapped when the results region holds a grid
- The class-instances, Geo and Latest dialogs left a 60px band under the pager; a flush dialog body runs a column down to the view body
- A dialog's form actions scrolled away with the fields; the bar sticks to the scrollport's bottom, and the form bar and the block foot stop being one band under two names
- The first save of prose nobody had edited rewrote the `rdf:XMLLiteral`, adding an `xmlns` to its first child: the div and its content were serialized in two namespace contexts and joined as strings. The wrapper is built as a node
- An XHTML content block's serialized literal longer than the JS stack allows (about 6–7K characters) took the whole client transform down with "too much recursion" in `distinct-values()`; `ldh:bounded-key()` folds long keys into prefix, length and hash
- The pencil on a committed object value replaced the chip with a combobox and kept nothing, so a pencil clicked by accident emptied the row; the chip is stashed and Escape or an empty focusout restores it
- `acl:mode()` inside a match pattern threw on the missing access-mode object and Saxon-JS discards a throw in a pattern, so click-to-edit on XHTML content and the Write-gated drag rules were dead until the first document response; the object is initialised empty
- A document describing two resources beside a content aside put the second resource in the gutter and let the aside scroll away after the first: the aside spans every row and every child names the content column
- An image in document content scales to its column instead of its pixel size, and keeps that size while the document is being edited rather than dropping to a thumbnail
- A `PATCH` 422 echoed the entire would-be document graph rather than the violating resources, so the client built a row form for every sibling and crashed on YASQE init; the response carries the violation roots' bounded descriptions
...
- Opening any document on an instance nobody had made public raised a modal alert with raw Saxon text: the tree descent walked the loading placeholder as a child. Navigation that cannot work now degrades quietly — a drawer section whose query is refused is removed rather than filled with an alert, breadcrumbs stop at the ancestor that could not be read, and Geo, Latest and search defer their query to the click and report a refusal in a modal
...
- An unbound `$Type` in the ACL query was a wildcard: a resource with no stored type — `/settings`, `/sparql`, a URL that does not exist — matched every `acl:accessToClass` authorization, so an unauthenticated agent could read `/settings` and `/sparql` and a writer could `PATCH` `/settings`. Both ACL queries carry a default `VALUES ?Type { rdfs:Resource }` that a typed resource's real types replace, so a typeless resource matches nothing. The one authorization that leaked through it — `oauth2-login`, granting `foaf:Agent` read via `acl:accessToClass` on two endpoint documents — moves to the end-user app as `acl:accessTo`. Net: `/settings` owner-only, `/sparql` authenticated-only, `/ns` and `/access` public, OAuth login public
