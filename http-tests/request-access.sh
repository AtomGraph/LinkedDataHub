...
curl -w "%{http_code}\n" -k -s \
  ...
  --data-urlencode "ou=https://localhost:4443/sparql" \
  --data-urlencode "pu=https://w3id.org/atomgraph/linkeddatahub/admin/acl/domain#requestAccessToClass" \
  --data-urlencode "ou=https://localhost:4443/ns/default#Root" \
  --data-urlencode "ou=https://localhost:4443/ns/default#Container" \
  --data-urlencode "ou=https://localhost:4443/ns/default#Item" \
  --data-urlencode "ou=https://localhost:4443/ns/default#File" \
  ...
  --data-urlencode "pu=http://www.w3.org/1999/02/22-rdf-syntax-ns#type" \
  --data-urlencode "ou=https://localhost:4443/admin/ns#AuthorizationRequestItem" \
