...
# LNK-009: Test that loopback IP (127.0.0.1) is blocked via SSRF protection
# Attempt to access 127.0.0.1 via the proxy
...
http_status=$(curl -k -s -o /dev/null -w "%{http_code}" \
  ...
  --data-urlencode "uri=http://127.0.0.1:8080/test" \
  "$END_USER_BASE_URL" || true)

# Verify that access was forbidden (403)
if [ "$http_status" != "403" ]; then
    echo "Expected HTTP 403 Forbidden for 127.0.0.1 access, got: $http_status"
    exit 1
fi
