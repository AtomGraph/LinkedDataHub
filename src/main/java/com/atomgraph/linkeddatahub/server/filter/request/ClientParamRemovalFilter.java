...
//    }
    ...
    {
        UriBuilder requestUriWithoutAC = UriBuilder.fromUri(request.getUriInfo().getRequestUri());
        requestUriWithoutAC.replaceQuery(null);

        Iterator<Map.Entry<String, List<String>>> it = request.getUriInfo().getQueryParameters().entrySet().iterator();
        while (it.hasNext())
        {
            Map.Entry<String, List<String>> entry = it.next();
            for (String value : entry.getValue())
        ...
        request.setRequestUri(request.getUriInfo().getBaseUri(), requestUriWithoutAC.build());
    }
