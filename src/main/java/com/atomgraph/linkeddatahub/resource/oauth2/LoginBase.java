...
public abstract class LoginBase
{
     ...
     *
     * @param defaultGraph default graph parameter (unused)
     * @param graphUri graph URI parameter (unused)
    ...
    public Response get(@QueryParam("default") @DefaultValue("false") Boolean defaultGraph, @QueryParam("graph") URI graphUri)
    {
                ...
                request().post(Entity.form(form)))
        {
            ...
            if (!accountExists) // UserAccount with this ID does not exist yet
            {
                ...
                agentPss.setLiteral(SIOC.ID.getLocalName(), jwt.getSubject());
                if (mbox.isPresent()) agentPss.setParam(FOAF.mbox.getLocalName(), mbox.get());
                else agentPss.setParam(FOAF.mbox.getLocalName(), RDFS.Resource); // disable the $mbox UNION branch if email is not provided

                final Model agentModel = getAgentService().getSPARQLClient().loadModel(agentPss.asQuery());
                // if Agent with this foaf:mbox or this account does not exist (lookup model is empty), create it; otherwise, reuse it
...
//            DecodedJWT jwt = verifier.verify(idToken);
     ...
     */
    public Resource createUserAccount(Model model, URI graphURI, Resource container, String id, String issuer, String name, Optional<String> email)
    {
        Resource item = model.createResource(graphURI.toString()).
            addProperty(RDF.type, DH.Item).
            addProperty(SIOC.HAS_CONTAINER, container).
