...
public class Executor
{
    ...
    public void start(CSVImport csvImport, com.atomgraph.linkeddatahub.server.model.Resource importRes, Resource provGraph, Service service, String baseURI, DataManager dataManager)
    {
        ...
        CompletableFuture.supplyAsync(fileSupplier, getExecutorService()).thenApplyAsync(getStreamRDFOutputWriter(csvImport,
            ...
            thenAcceptAsync(success(csvImport, importRes, provImport, provGraph, service, dataManager), getExecutorService()).
            exceptionally(failure(csvImport, importRes, provImport, provGraph, service));
    }

    public void start(RDFImport rdfImport, com.atomgraph.linkeddatahub.server.model.Resource importRes, Resource provGraph, Service service, String baseURI, DataManager dataManager)
    {
        ...
        CompletableFuture.supplyAsync(fileSupplier, getExecutorService()).thenApplyAsync(getStreamRDFOutputWriter(rdfImport,
                dataManager, baseURI, pss.asQuery()), getExecutorService()).
            thenAcceptAsync(success(rdfImport, importRes, provImport, provGraph, service, dataManager), getExecutorService()).
            exceptionally(failure(rdfImport, importRes, provImport, provGraph, service));
    }
    
    protected Consumer<CSVStreamRDFOutput> success(final CSVImport csvImport, final com.atomgraph.linkeddatahub.server.model.Resource importRes, final Resource provImport, final Resource provGraph, final Service service, final DataManager dataManager)
    {
        return (CSVStreamRDFOutput output) ->
        {
            ...
            if (service.getProxy() != null) ban(dataManager, service.getProxy(), csvImport.getContainer().getURI()); // purge cache entries that include the target container URL
        };
    }
    
    protected Consumer<StreamRDFOutput> success(final RDFImport rdfImport, final com.atomgraph.linkeddatahub.server.model.Resource importRes, final Resource provImport, final Resource provGraph, final Service service, final DataManager dataManager)
    {
...
//                    addLiteral(VoID.triples, output.getCSVStreamRDFProcessor().getTripleCount()).
            ...
            if (service.getProxy() != null) ban(dataManager, service.getProxy(), rdfImport.getContainer().getURI()); // purge cache entries that include the target container URL
        };
    }
