...
exclude-result-prefixes="#all">
    ...
    <xsl:template match="*[rdf:type/@rdf:resource = '&adm;SignUp'][$ac:method = 'GET']" mode="bs2:Row" priority="2">
        <xsl:apply-templates select="ldh:construct(map{ xs:anyURI('&foaf;Person'): ldh:query-result(xs:anyURI('&foaf;Person'), resolve-uri('ns', $ldt:base), $constructor-query)//srx:binding[@name = 'construct']/srx:literal/string() })" mode="bs2:RowForm">
