...
>
    ...
    <xsl:template match="rdf:RDF" mode="schema:BreadCrumbList">
        <xsl:variable name="resource" select="key('resources', ldh:absolute-path(base-uri()))" as="element()?"/>
        ...
        <div class="row-fluid">
            <ul class="nav nav-tabs offset2 span7">
                <li class="content-mode{if ((empty($active-mode) and $has-content and not($forClass)) or $active-mode = '&ldh;ContentMode') then ' active' else() }">
                    <a href="{ldh:href($ldt:base, ldh:absolute-path(ldh:absolute-path(base-uri())), ldh:query-params(xs:anyURI('&ldh;ContentMode')), ldh:absolute-path(base-uri()))}">
                        <xsl:value-of>
    ...
    <xsl:template match="rdf:RDF" mode="ldh:ContentList">
        <xsl:apply-templates select="key('resources', ldh:absolute-path(base-uri()))" mode="#current"/>
    ...
    <xsl:template match="rdf:RDF" mode="bs2:Row">
        ...
        <xsl:apply-templates select="*[not(@rdf:about = ldh:absolute-path(base-uri()) and rdf:type/@rdf:resource = ('&def;Root', '&dh;Container', '&dh;Item')) and not(rdf:type/@rdf:resource = '&ldh;Content')]" mode="#current">
...
<!--                            <xsl:when test="$property/sh:nodeKind/@rdf:resource = '&sh;Literal'">
    ...
    <xsl:template match="rdf:RDF" mode="bs2:RowForm">
        <xsl:param name="method" select="'post'" as="xs:string"/>
        <xsl:param name="action" select="ldh:href($ldt:base, ldh:absolute-path(base-uri()), map{}, ac:build-uri(base-uri(), map{ '_method': 'PUT', 'mode': for $mode in $ac:mode return string($mode) }))" as="xs:anyURI"/>
        <xsl:param name="id" select="concat('form-', generate-id())" as="xs:string?"/>
        <xsl:param name="class" select="'form-horizontal'" as="xs:string?"/>
        <xsl:param name="accept-charset" select="'UTF-8'" as="xs:string?"/>
        <xsl:param name="enctype" select="'multipart/form-data'" as="xs:string?"/>
        ...
        <form method="{$method}" action="{$action}">
            ...
            <xsl:apply-templates select="*[@rdf:about = ldh:absolute-path(base-uri())]" mode="#current">
                <xsl:with-param name="inline" select="false()" tunnel="yes"/>
            </xsl:apply-templates>
            <!-- show the rest of the resources (contents, instances) below it -->
            <xsl:apply-templates select="*[not(@rdf:about = ldh:absolute-path(base-uri()))]" mode="#current">
                <xsl:sort select="ac:label(.)"/>
                <xsl:with-param name="inline" select="false()" tunnel="yes"/>
            </xsl:apply-templates>
            ...
            <xsl:if test="$create-resource">
                <div class="create-resource row-fluid">
                    <div class="main offset2 span7">
                        ...
                        <a href="{ac:build-uri(ldh:absolute-path(base-uri()), map{ 'forClass': '&ldh;Content' })}" class="btn btn-primary add-constructor create-action">
                            <xsl:value-of>
        ...
        <div>
            ...
            <ul class="dropdown-menu">
                <xsl:if test="$show-document-classes">
                    ...
                    <xsl:if test="exists($document-classes) and key('resources', ldh:absolute-path(base-uri()))/rdf:type/@rdf:resource = ('&def;Root', '&dh;Container')">
