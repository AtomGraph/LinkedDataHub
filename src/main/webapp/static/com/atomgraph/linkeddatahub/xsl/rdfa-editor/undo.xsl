...
<!--
    ...
    Every mutating handler pushes a snapshot first and calls rdfae:after-mutation last;
    plain typing is
...
-->
    ...
    <xsl:variable name="rdfae:max-undo" as="xs:integer" select="100"/>

    <xsl:template name="rdfae:init-undo">
    ...
    <xsl:template name="rdfae:stash-push">
        <xsl:param name="stack" as="element()"/>
        <xsl:param name="snapshot" as="xs:string"/>
        <xsl:param name="root" as="element()"/>

        <xsl:variable name="entry" as="element()" select="rdfae:element('div')"/>
        ...
        <ixsl:set-attribute name="data-root"
            select="string(count(rdfae:roots()[. &lt;&lt; $root]) + 1)" object="$entry"/>
        <xsl:call-template name="rdfae:capture-caret">
            <xsl:with-param name="entry" select="$entry"/>
            <xsl:with-param name="root" select="$root"/>
        </xsl:call-template>
        <xsl:sequence select="ixsl:call($stack, 'appendChild', [ $entry ])[current-date() lt xs:date('2000-01-01')]"/>
        <xsl:for-each select="($stack/div)[position() le count($stack/div) - $rdfae:max-undo]">
    ...
    <xsl:template name="rdfae:capture-caret">
        ...
        <xsl:if test="ixsl:get($selection, 'rangeCount') ge 1">
            ...
            <xsl:if test="ixsl:get($anchor, 'nodeType') = 3 and (rdfae:root-of($anchor) ! (. is $root))">
                <xsl:for-each select="rdfae:block-of($anchor)">
    ...
    <xsl:template name="rdfae:push-undo">
        <xsl:param name="host" as="element()?" select="()"/>
        <xsl:param name="root" as="element()?" select="($host ! rdfae:root-of(.), rdfae:active-root())[1]"/>
        ...
        <xsl:for-each select="$root[exists($snapshot)]">
            ...
            <xsl:variable name="root-index" as="xs:integer" select="count(rdfae:roots()[. &lt;&lt; current()]) + 1"/>
            ...
            <xsl:if test="not(string($top) eq $snapshot and xs:integer($top/@data-root) eq $root-index)">
                <xsl:call-template name="rdfae:stash-push">
        ...
        <ixsl:set-property name="lastUndoTime"
            select="ixsl:call(ixsl:get(ixsl:window(), 'Date'), 'now', [])" object="rdfae:editor-state()"/>
        <ixsl:set-property name="lastUndoHost" select="$host" object="rdfae:editor-state()"/>
    </xsl:template>
    ...
    <!-- undo and redo are the same move with the stacks swapped: pop $from,
         ...
         clears the redo stack - only rdfae:push-undo does), restore -->
    <xsl:template name="rdfae:shift-history">
        ...
        <xsl:for-each select="(id($from, ixsl:page())/div)[last()]">
            ...
            <xsl:variable name="root" as="element()?" select="rdfae:roots()[$root-index]"/>
            <xsl:call-template name="rdfae:stash-push">
            ...
            <xsl:sequence select="ixsl:call(., 'remove', [])[current-date() lt xs:date('2000-01-01')]"/>
            <xsl:call-template name="rdfae:restore-snapshot">
                <xsl:with-param name="snapshot" select="$snapshot"/>
    ...
    <xsl:template name="rdfae:apply-undo">
        <xsl:call-template name="rdfae:shift-history">
            <xsl:with-param name="from" select="'rdfa-editor-undo-stack'"/>
            <xsl:with-param name="to" select="'rdfa-editor-redo-stack'"/>
        </xsl:call-template>
    </xsl:template>

    <xsl:template name="rdfae:apply-redo">
        <xsl:call-template name="rdfae:shift-history">
    ...
    <xsl:template name="rdfae:restore-snapshot">
        <xsl:param name="snapshot" as="xs:string"/>
        ...
        <xsl:for-each select="$root">
            ...
            <xsl:call-template name="rdfae:ensure-chrome">
            ...
            <xsl:for-each select="descendant-or-self::*[rdfae:island(.)][empty(*[@data-role = 'rendering'])]">
                <xsl:apply-templates select="." mode="rdfae:render-island"/>
            </xsl:for-each>
        </xsl:for-each>
        ...
        <ixsl:set-property name="lastUndoTime" select="0" object="rdfae:editor-state()"/>
        ...
        <xsl:choose>
            <xsl:when test="exists($target)">
                <xsl:for-each select="rdfae:host-of($target)">
        ...
        <xsl:if test="exists(rdfae:block-of(.))">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:if test="$now - xs:double((ixsl:get(rdfae:editor-state(), 'lastUndoTime'), 0)[1]) gt 1000
                            or not(ixsl:get(rdfae:editor-state(), 'lastUndoHost') ! (. is current()))">
    ...
    <xsl:template name="rdfae:after-mutation">
        <xsl:call-template name="rdfae:ensure-chrome"/>
