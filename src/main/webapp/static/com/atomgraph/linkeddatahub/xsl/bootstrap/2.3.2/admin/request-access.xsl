...
exclude-result-prefixes="#all">
    ...
    <xsl:template match="rdf:RDF[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)][$ac:method = 'POST'][key('resources-by-type', '&spin;ConstraintViolation') or key('resources-by-type', '&sh;ValidationResult')]" mode="xhtml:Body" priority="3">
    ...
    <xsl:template match="rdf:RDF[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)][$ac:method = 'GET']" mode="bs2:ContentBody" priority="2">
    ...
    <xsl:template match="rdf:RDF[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:ModeTabs" priority="2"/>

    <!-- disable the right nav (backlinks etc.) -->
    <xsl:template match="*[*][@rdf:about or @rdf:nodeID][ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:Right" use-when="system-property('xsl:product-name') = 'SAXON'"/>
            ...
            <xsl:document>
                ...
                <xsl:for-each select="ldh:construct(map{ xs:anyURI('&dh;Item'): $constructors//srx:result[srx:binding[@name = 'Type'] = '&dh;Item']/srx:binding[@name = 'construct']/srx:literal/string(),
                    ...
                    <xsl:apply-templates select="." mode="ldh:SetPrimaryTopic">
                        ...
                        <xsl:with-param name="doc-uri" select="xs:anyURI('')" tunnel="yes"/> <!-- empty relative URI (i.e. should resolve to the target document URL) -->
                    </xsl:apply-templates>
                </xsl:for-each>
            </xsl:document>
        </xsl:variable>
    ...
    <xsl:template match="*[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)][$ac:method = 'POST'][not(key('resources-by-type', '&http;Response'))]" mode="bs2:Row" priority="2"/>

    <!-- hide object blank nodes (that only have a single rdf:type property) from constructed models -->
    <xsl:template match="rdf:Description[$ac:method = 'GET'][not(rdf:type/@rdf:resource = ('&lacl;AuthorizationRequest', '&dh;Item'))][ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:RowForm" priority="3"/>

    <xsl:template match="*[*][@rdf:about or @rdf:nodeID][ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:FormControl" priority="1">
        <xsl:next-match>
    ...
    <xsl:template match="*[@rdf:about or @rdf:nodeID][ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]/sioc:has_parent | *[@rdf:about or @rdf:nodeID][ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]/sioc:has_container" mode="bs2:FormControl" priority="4">
    ...
    <xsl:template match="lacl:requestMode/@rdf:*[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:FormControl" priority="2">
    ...
    <xsl:template match="lacl:requestAgent[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:FormControl" priority="2">
        <xsl:apply-templates select="." mode="xhtml:Input">
            <xsl:with-param name="type" select="'hidden'"/>
        </xsl:apply-templates>
    ...
    <xsl:template match="lacl:requestAccessTo/@rdf:*[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)][$ldh:access-to]" mode="bs2:FormControl" priority="2">
    ...
    <xsl:template match="lacl:requestAccessToClass[1]/@rdf:*[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:FormControl" priority="2">
        <xsl:variable name="this" select="../concat(namespace-uri(), local-name())" as="xs:string"/>
    ...
    <xsl:template match="lacl:requestAccessToClass/@rdf:*[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:FormControl" priority="1"/>
    
    <!-- hide type control -->
    <xsl:template match="*[*][@rdf:about or @rdf:nodeID][ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:TypeControl" priority="2">
        <xsl:next-match>
    ...
    <!-- suppress properties -->
    <xsl:template match="dct:title[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)] | dct:description[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)] | rdf:_1[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:FormControl" priority="4"/>

    <!-- hide properties (including all of document resource properties) -->
    <xsl:template match="rdfs:label[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)] | foaf:isPrimaryTopicOf[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)] | *[foaf:primaryTopic][ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]/*" mode="bs2:FormControl" priority="3">
        <xsl:apply-templates select="." mode="xhtml:Input">
            <xsl:with-param name="type" select="'hidden'"/>
        </xsl:apply-templates>
    ...
    <xsl:template match="*[ac:absolute-path($ldh:requestUri) = resolve-uri(encode-for-uri('request access'), $ldt:base)]" mode="bs2:PropertyControl" priority="1"/>
