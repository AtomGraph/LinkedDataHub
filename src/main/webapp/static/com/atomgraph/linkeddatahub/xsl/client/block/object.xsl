...
>
    ...
    <!-- hide type control -->
    <xsl:template match="*[rdf:type/@rdf:resource = '&ldh;Object']" mode="ldh:TypeControl" priority="1">
        <xsl:next-match>
    ...
    <xsl:template match="*[rdf:type/@rdf:resource = '&ldh;Object']/rdfs:label | *[rdf:type/@rdf:resource = '&ldh;Object']/ac:mode" mode="ac:FormControl">
        <xsl:next-match>
    ...
    <xsl:template match="*[@typeof = '&ldh;Object'][descendant::*[@property = '&rdf;value'][@resource]]" mode="ldh:RowHook" as="function(item()?) as map(*)">
                    ...
                    <xsl:for-each select="?body">
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when test="$resource">
                                <xsl:variable name="object-uris" select="distinct-values($resource/*/@rdf:resource[starts-with(., lds:base())][not(key('resources', ., root($resource)))])" as="xs:string*"/>
                                ...
                                <xsl:variable name="ns-request" select="map{ 'method': 'POST', 'href': ldh:href(resolve-uri('ns', lds:base())), 'media-type': 'application/sparql-query', 'body': $ns-query-string, 'headers': map{ 'Accept': 'application/rdf+xml' } }" as="map(*)"/>
        ...
        <xsl:choose>
            <xsl:when test="$resource">
                <xsl:for-each select="$response">
                    ...
                    <!-- both are labels only, so either failing renders the block all the same, its objects labelled the way an undescribed one already is -->
                    <xsl:variable name="sparql-metadata" select="if (?status = 200 and ?media-type = 'application/rdf+xml') then ?body else ()" as="document-node()?"/>
                    <xsl:variable name="object-metadata" select="ldh:merge-metadata($sparql-metadata, $ns-metadata)" as="document-node()"/>

                    <xsl:variable name="row" as="node()*">
                        <xsl:apply-templates select="$resource" mode="ldh:BlockRow">
