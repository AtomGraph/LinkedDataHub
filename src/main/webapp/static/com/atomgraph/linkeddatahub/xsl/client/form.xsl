...
WHERE
  {   { ?class (rdfs:subClassOf)*/spin:constructor ?constructor
    ...
    <xsl:template match="*[rdf:type/@rdf:resource = $system-types]" mode="ldh:RowForm" priority="3"/>

    <!-- suppress constraint violations and HTTP responses in the form - they are displayed as errors on the edited resources -->
    <xsl:template match="*[rdf:type/@rdf:resource = $system-types]" mode="ac:ResourceForm" priority="3"/>
    ...
    <xsl:template match="select[contains-token(@class, 'subject-type')]" mode="ixsl:onchange">
        ...
        <xsl:variable name="subject-row" select="ancestor::div[contains-token(@class, 'ldh-subject')][1]" as="element()"/>
        <xsl:variable name="old-subject-type" select="$subject-row//input[contains-token(@class, 'subject-type')][contains-token(@class, 'old')]" as="element()"/> <!-- old value in a hidden input -->
        <xsl:variable name="old-type" select="string(ixsl:get($old-subject-type, 'value'))" as="xs:string"/>
        <xsl:variable name="value" select="string(ixsl:get($subject-row//input[contains-token(@class, 'subject')], 'value'))" as="xs:string"/>
        <xsl:variable name="new-type-old-subject" select="$subject-row//input[contains-token(@class, 'old')][contains-token(@class, $new-type)]" as="element()"/> <!-- old value (of the new type) in a hidden input -->
    ...
    <xsl:template match="fieldset//input[contains-token(@class, 'subject')]" mode="ixsl:onchange">
        ...
        <xsl:variable name="subject-row" select="ancestor::div[contains-token(@class, 'ldh-subject')][1]" as="element()"/>
        <xsl:variable name="subject-type" select="string(ixsl:get($subject-row//select[contains-token(@class, 'subject-type')], 'value'))" as="xs:string"/> <!-- "su" (URI) or "sb" (bnode) -->
        <xsl:variable name="old-subject" select="$subject-row//input[contains-token(@class, 'old')][contains-token(@class, $subject-type)]" as="element()"/>
    ...
    <xsl:template match="div[contains-token(@class, 'rdfa-editor-content')][empty(id('edit-toolbar', ixsl:page()))]" mode="ldh:RenderRowForm" priority="2">
        ...
        <!-- canonical form at activation: the autosave compares against it and skips the no-op save.
             Stashed after init, which seeds an empty region with a caret paragraph - a baseline taken
             before it would leave an untouched fresh block permanently dirty -->
    ...
    <xsl:template match="div[contains-token(@class, 'rdfa-editor-content')]" mode="ldh:RenderRowForm" priority="1">
        ...
        <!-- the activation baseline follows init here too, for the same seeded-paragraph reason -->
        <ixsl:set-property name="canonicalBaseline" select="ldh:canonical-content(.)" object="."/>
        <xsl:call-template name="ldh:focus-editable">
            <xsl:with-param name="region" select="."/>
        </xsl:call-template>
    ...
    <xsl:template match="input[@type = 'datetime-local'][ixsl:get(., 'value')]" mode="ldh:FormPreSubmit" priority="1">
        ...
        <!-- the value and timezone inputs sit in separate field boxes, so navigate via the shared .ldh-dt-pair rather than the sibling axis -->
        <xsl:variable name="timezone" select="ixsl:get(ancestor::span[contains-token(@class, 'ldh-dt-pair')][1]//input[contains-token(@class, 'input-timezone')], 'value')" as="xs:string"/>
        ...
        <xsl:for-each select="$content-body">
            <xsl:result-document href="?." method="ixsl:replace-content">
                ...
                <xsl:for-each select="document($request-uri)">
                    <xsl:apply-templates select="key('resources-by-type', '&foaf;Person')[@rdf:about]" mode="ldh:BlockRow"/>
                </xsl:for-each>
            </xsl:result-document>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:function name="ldh:load-constraints" as="map(*)" ixsl:updating="yes">
        ...
        <xsl:variable name="request" select="map{ 'method': 'POST', 'href': ldh:href(resolve-uri('ns', lds:base())), 'media-type': 'application/sparql-query', 'body': $query-string, 'headers': map{ 'Accept': 'application/sparql-results+xml' } }" as="map(*)"/>
            ...
            <xsl:choose>
                <xsl:when test="?status = 200 and ?media-type = 'application/rdf+xml'">
                    ...
                    <xsl:for-each select="?body">
                        ...
                        <xsl:variable name="results-uri" select="ac:build-uri(resolve-uri('ns', lds:base()), map{ 'query': $query-string, 'accept': 'application/rdf+xml' })" as="xs:anyURI"/>
    ...
    <!-- Fold step for the create-new-instance chain. Instantiates the (pure, bnode-prototyped) constructor under the minted URI and extracts the row-form resource + its types. Reads context('constructed-doc'), context('forClass'), context('this'); writes context('instance-doc'), context('resource'), context('types'). context('constructed-doc') stays the pure constructor — ac:FormControl reads it via the 'constructor' tunnel to derive $template / the property dropdown. -->
    <xsl:function name="ldh:set-row-form-resource" as="map(*)" ixsl:updating="yes">
        ...
        <xsl:variable name="instance-doc" select="ldh:instantiate-constructor($constructed-doc, $forClass, $this, ())" as="document-node()"/>

        <xsl:variable name="resource" select="key('resources-by-type', $forClass, $instance-doc)[rdf:type/@rdf:resource = '&owl;NamedIndividual' or (not(key('predicates-by-object', @rdf:nodeID)) and exists(* except rdf:type))]" as="element()"/>
    ...
    <xsl:function name="ldh:render-add-row-form" as="item()*" ixsl:updating="yes">
        ...
        <xsl:variable name="row-form" as="element()">
            ...
            <xsl:variable name="query-string" select="'DESCRIBE $Type VALUES $Type { ' || string-join(for $type in $types return '&lt;' || $type || '&gt;', ' ') || ' }'" as="xs:string"/>
            <xsl:variable name="request-uri" select="ldh:href(ac:build-uri(resolve-uri('ns', lds:base()), map{ 'query': $query-string, 'accept': 'application/rdf+xml' }), map{})" as="xs:anyURI"/>
            ...
            <xsl:variable name="request-uri" select="ldh:href(ac:build-uri(resolve-uri('ns', lds:base()), map{ 'query': $query-string, 'accept': 'application/sparql-results+xml' }), map{})" as="xs:anyURI"/>
    ...
    <!-- per-Description dispatch: keep the editable subject, suppress every other Description in the response graph. The discriminator $about is a tunnel param because XSLT 3.0 patterns cannot reference template-local params; the keep-or-suppress test lives in the body via xsl:if. Dispatches to ac:ResourceForm mode so the ac:ResourceForm mode template at resource.xsl:1182 handles the ac:FormControl entry with the right $inline / $required params and the shell's default merged-properties iteration (resource props + constructor template props, sorted by constraints, with the violations/constructor/type-constraints/type-shapes with-params required by per-property templates). -->
    ...
    <!-- thin dispatcher: function-item invoked by the initial-render and violation handlers via ldh:render-document-form($body, $ctx). Named (not an inline XPath function literal) because xsl:apply-templates is an XSLT instruction. base-uri falls back to the editable subject URI when the caller hasn't set it, so ac:ResourceForm's xs:anyURI typecheck doesn't see an empty value when the body is a freshly constructed doc with no associated URI. The 'constructor' tunnel folds ctx('shapes') + ctx('constructed-doc') via ldh:build-merged-constructor — same pure-constructor input to ac:FormControl as the inline row-form renders, so the property picker offers the classes' constructor properties. -->
    ...
    <!-- thin dispatcher for the Container/Item creation flow (the SPIN-constructor-driven "add" form). Mirrors ldh:render-document-form#2 but dispatches in mode="ac:ResourceForm" instead of mode="ldh:DocumentForm". ac:ResourceForm's rdf:RDF shell at document.xsl:1170 iterates all Descriptions (primary at @rdf:about=$base-uri + non-primary sorted by label) — exactly what the constructor flow needs to keep co-shipped content blocks visible across the initial render and the constraint-violation re-render. http:Response and spin:ConstraintViolation Descriptions are suppressed by the existing priority=3 empty templates at form.xsl:106/109. -->
    <xsl:function name="ldh:render-constructor-form" as="element()*">
        <xsl:param name="body" as="document-node()"/>
        <xsl:param name="ctx"  as="map(*)"/>
        <xsl:apply-templates select="$body" mode="ac:ResourceForm">
            <xsl:with-param name="method"            select="$ctx('method')"/>
            <xsl:with-param name="action"            select="$ctx('action')"            tunnel="yes"/>
            <xsl:with-param name="base-uri"          select="if (map:contains($ctx, 'base-uri')) then $ctx('base-uri') else $ctx('about')" tunnel="yes"/>
            <xsl:with-param name="type-metadata"     select="$ctx('type-metadata')"     tunnel="yes"/>
    ...
    <xsl:template match="div[contains-token(@class, 'block')][@about]" mode="ldh:CancelEditing">
    ...
    <xsl:template match="form[ancestor::div[@typeof]][tokenize(@class, ' ') = ('ldh-prop-form', 'ldh-edit-form')]" mode="ixsl:onsubmit">
    ...
    <xsl:function name="ldh:render-add-value" as="item()*" ixsl:updating="yes">
        ...
        <xsl:for-each select="$prop-form">
            <xsl:result-document href="?." method="ixsl:append-content">
                <xsl:choose>
                    <!-- TO-DO: unify ldh:TypeControl and ac:FormControl? -->
                    <xsl:when test="$property-uri = '&rdf;type'">
                        <xsl:apply-templates select="$property" mode="ldh:TypeControl">
                    ...
                    <xsl:otherwise>
                        <xsl:apply-templates select="$property" mode="ac:FormControl">
                            <!-- generate fresh $for value because otherwise we can generate existing IDs from the same constructor -->
                            <xsl:with-param name="for" select="'id' || ac:uuid()"/>
                        </xsl:apply-templates>
                    </xsl:otherwise>
            ...
            <!-- initialize the new group after it's appended -->
            <xsl:apply-templates select="*[last()]" mode="ldh:RenderRowForm"/>
        </xsl:for-each>
    ...
    <xsl:template name="ldh:SyncFormsWithConstructor">
        <xsl:apply-templates select="ixsl:page()//fieldset[./div[contains-token(@class, 'ldh-prop-form')]/div[contains-token(@class, 'ldh-prop-group')]/input[@name = 'pu']][ancestor::div[@typeof][contains-token(@class, 'block')]]" mode="ldh:SyncFormWithConstructor"/>
    </xsl:template>
    ...
    <xsl:function name="ldh:sync-form-with-constructor" as="item()*" ixsl:updating="yes">
        ...
        <xsl:if test="map:contains($context, 'constructed-doc')">
            ...
            <xsl:variable name="prop-form" select="$fieldset/div[contains-token(@class, 'ldh-prop-form')]" as="element()"/>
            <xsl:variable name="existing-uris" select="$prop-form/div[contains-token(@class, 'ldh-prop-group')]/input[@name = 'pu']/@value" as="xs:string*"/>
            <xsl:variable name="new-arcs" select="$arcs[not(concat(namespace-uri(), local-name()) = $existing-uris)]" as="element()*"/>

            <!-- remove value-less groups whose property the constructor no longer asserts -->
            <xsl:for-each select="$prop-form/div[contains-token(@class, 'ldh-prop-group')][input[@name = 'pu']/@value[not(. = ($arc-uris, '&rdf;type'))]][empty(descendant::*[self::input or self::textarea][@name = ('ou', 'ol', 'ob')][ixsl:get(., 'value') ne ''])]">
                <xsl:sequence select="ixsl:call(., 'remove', [])[current-date() lt xs:date('2000-01-01')]"/>
            </xsl:for-each>
            ...
            <xsl:if test="exists($new-arcs)">
                ...
                <xsl:variable name="group-count" select="count($prop-form/div[contains-token(@class, 'ldh-prop-group')])" as="xs:integer"/>
                ...
                <xsl:for-each select="$prop-form">
                    ...
                    <xsl:apply-templates select="./div[contains-token(@class, 'ldh-prop-group')][position() gt $group-count]" mode="ldh:RenderRowForm"/>
                </xsl:for-each>
            </xsl:if>
        </xsl:if>
    </xsl:function>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:variable name="new-block" as="element()">
                        <xsl:apply-templates select="$resources/rdf:RDF/*" mode="ldh:BlockRow">
    ...
    <xsl:function name="ldh:row-form-submit-violation" ixsl:updating="yes">
        ...
        <!-- the type set comes from the block's RDFa @typeof (stamped at render time by ldh:BlockRow and the create flows; the submit handler's match pattern guarantees it), not from the response body: the 422 echo carries auxiliary descriptions - inlined bnodes like the signup cert key, constraint descriptions referenced via spin:violationSource - whose types would pollute the constructor's forClass and fail ac:FormControl's prototype subset test. Trade-off: a type added via the type control just before submit is missing from @typeof until the next successful render -->
        <xsl:variable name="block" select="$context('block')" as="element()"/>
        ...
        <!-- $new-context is built synchronously above; types/forClass/property-uris/object-uris populated from the violation response body. No pre-baked type-metadata-request here, so the type-metadata pair uses the normal ldh:load-type-metadata. The ldh:load-constructed-doc pair fetches the SPIN constructor so ldh:render-row-form-violation can merge it with $shapes via ldh:build-merged-constructor — same input shape as the initial CREATE / EDIT renders. -->
    ...
    <xsl:function name="ldh:render-row-form-violation" as="item()*" ixsl:updating="yes">
        ...
        <xsl:variable name="row-form" as="node()*">
            ...
            <xsl:apply-templates select="$body/rdf:RDF/*[not(@rdf:about = $doc-uri)]" mode="ldh:RowForm">
    ...
    <!-- toggles the .ldh-subject row for subject URI/bnode ID editing -->
    <xsl:template match="button[contains-token(@class, 'btn-edit-subj')]" mode="ixsl:onclick">
        <!-- the subject row immediately follows the .ldh-form-subjbar hosting the toggle -->
        <xsl:variable name="subject-row" select="ancestor::div[contains-token(@class, 'ldh-form-subjbar')][1]/following-sibling::div[1][contains-token(@class, 'ldh-subject')]" as="element()"/>
        <xsl:variable name="pressed" select="ixsl:get(., 'ariaPressed') = 'true'" as="xs:boolean"/>

        <ixsl:set-property name="ariaPressed" select="if ($pressed) then 'false' else 'true'" object="."/>

        <xsl:for-each select="$subject-row">
            <xsl:choose>
                <!-- clearing the inline display hands the choice back to the stylesheet (grid in form mode, flex standalone) -->
    ...
    <xsl:function name="ldh:set-combobox-form-resource" as="map(*)" ixsl:updating="yes">
        ...
        <xsl:variable name="merged-constructor" select="ldh:merge-constructors($shape-instance-doc, $constructed-doc, $forClass)" as="document-node()"/>
        <xsl:variable name="instance-doc" select="ldh:instantiate-constructor($merged-constructor, $forClass, $this, ())" as="document-node()"/>

        <xsl:variable name="resource" select="key('resources-by-type', $forClass, $instance-doc)[not(key('predicates-by-object', @rdf:nodeID))]" as="element()"/>
        <xsl:variable name="types" select="for $t in distinct-values($resource/rdf:type/@rdf:resource) return xs:anyURI($t)" as="xs:anyURI*"/>

        <xsl:sequence select="map:merge(($context, map{
            'constructed-doc': $merged-constructor,
            'instance-doc': $instance-doc,
            'shapes': $shapes,
            'resource': $resource,
            'types': $types,
    ...
    <xsl:function name="ldh:render-combobox-row-form" as="item()*" ixsl:updating="yes">
        ...
        <xsl:for-each select="$fieldset">
            <xsl:variable name="new-fieldset" as="element()*">
                <xsl:apply-templates select="$resource" mode="ac:ResourceForm">
    ...
    <!-- remove the statement: a property group hosts a single row, so the whole group goes (label included); a constructor row is self-contained -->
    <xsl:template match="button[contains-token(@class, 'btn-remove-property')]" mode="ixsl:onclick" priority="1">
        <xsl:sequence select="ixsl:call((ancestor::div[contains-token(@class, 'ldh-ctor-row')], ancestor::div[contains-token(@class, 'ldh-prop-group')])[1], 'remove', [])[current-date() lt xs:date('2000-01-01')]"/>
    </xsl:template>
    ...
    <xsl:template match="div[contains-token(@class, 'ac-fileinput')]//input[@type = 'file']" mode="ixsl:onchange">
        ...
        <xsl:for-each select="$wrapper/div[contains-token(@class, 'ldh-file-sel')]">
            <xsl:sequence select="ixsl:call(., 'remove', [])[current-date() lt xs:date('2000-01-01')]"/>
        </xsl:for-each>

        <xsl:if test="exists($file-name)">
            <xsl:for-each select="$wrapper">
                <xsl:result-document href="?." method="ixsl:append-content">
                    <div class="ldh-file-sel">
                        <span class="msi outline sm" aria-hidden="true">description</span>
                        <span class="fs-name">
