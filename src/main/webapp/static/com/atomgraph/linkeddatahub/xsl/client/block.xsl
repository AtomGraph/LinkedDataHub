...
>
    ...
    <!-- hide type control -->
    <xsl:template match="*[rdf:type/@rdf:resource = '&ldh;XHTML']" mode="ldh:TypeControl" priority="1">
        <xsl:next-match>
    ...
    <xsl:template match="*[rdf:type/@rdf:resource = '&ldh;XHTML']" mode="ac:FormControl" priority="1">
        <xsl:next-match>
    ...
    <xsl:template match="*[rdf:type/@rdf:resource = '&ldh;XHTML']/rdfs:label | *[rdf:type/@rdf:resource = '&ldh;XHTML']/ac:mode" mode="ac:FormControl">
        <xsl:next-match>
            <xsl:with-param name="label" select="ac:property-label(.)"/>
        </xsl:next-match>
    </xsl:template>

    <xsl:template match="*[rdf:type/@rdf:resource = '&ldh;XHTML']/rdf:value/xhtml:*" mode="ac:ValueAnnotations" priority="1"/>
    ...
    <xsl:template match="*[rdf:type/@rdf:resource = '&ldh;XHTML']" mode="ldh:RowForm" priority="1">
        ...
        <xsl:choose>
            <xsl:when test="rdf:type/@rdf:resource = $forClass">
                ...
                <!-- A property the caller left blank carries no value to hand over, so it does not displace
                     ...
                     the slot over with a valueless property loses the control entirely, since ac:FormControl
    ...
    <!-- ontology-view block injection: ontology query (?property ldh:view ?block) → per-URI RDF fetch → render via ldh:BlockRow → insert + hydrate -->
