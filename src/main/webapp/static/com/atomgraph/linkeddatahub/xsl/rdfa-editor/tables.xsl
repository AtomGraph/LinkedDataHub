...
-->
    ...
    <xsl:function name="local:cell-of" as="element()?">
        <xsl:param name="node"/>
    ...
    <xsl:function name="local:table-rows" as="element()*">
        <xsl:param name="table" as="element()?"/>
        <xsl:sequence select="$table/(thead | tbody | tfoot)/tr | $table/tr"/>
    </xsl:function>

    <xsl:function name="local:table-cells" as="element()*">
        <xsl:param name="table" as="element()?"/>
        <xsl:sequence select="local:table-rows($table)/(td | th)"/>
    </xsl:function>

    <xsl:function name="local:column-index" as="xs:integer">
        <xsl:param name="cell" as="element()"/>
        <xsl:sequence select="count($cell/preceding-sibling::*[self::td or self::th]) + 1"/>
    </xsl:function>

    <xsl:function name="local:has-spans" as="xs:boolean">
        <xsl:param name="table" as="element()?"/>
        <xsl:sequence select="exists(local:table-cells($table)[@colspan or @rowspan])"/>
    </xsl:function>
    ...
    <xsl:template name="local:make-cell">
        <xsl:param name="name" as="xs:string"/>

        <xsl:variable name="cell" as="element()" select="local:element($name)"/>
        <ixsl:set-attribute name="contenteditable" select="'true'" object="$cell"/>
        <xsl:sequence select="ixsl:call($cell, 'appendChild', [ local:element('br') ])[current-date() lt xs:date('2000-01-01')]"/>
        <xsl:sequence select="$cell"/>
    </xsl:template>

    <xsl:template name="local:make-row">
        <xsl:param name="cols" as="xs:integer"/>

        <xsl:variable name="row" as="element()" select="local:element('tr')"/>
        <xsl:for-each select="1 to $cols">
            <xsl:variable name="cell" as="element()">
                <xsl:call-template name="local:make-cell">
    ...
    <xsl:template name="local:insert-row">
        ...
        <xsl:for-each select="$anchor">
            <xsl:variable name="new" as="element()">
                <xsl:call-template name="local:make-row">
    ...
    <xsl:template match="button[contains-token(@class, 'table-save')]" mode="ixsl:onclick">
        ...
        <xsl:if test="$rows-raw castable as xs:integer and $cols-raw castable as xs:integer">
            ...
            <xsl:if test="$caption-text ne ''">
                <xsl:variable name="caption" as="element()" select="local:element('caption')"/>
            ...
            <xsl:if test="$header">
                <xsl:variable name="thead" as="element()" select="local:element('thead')"/>
                <xsl:variable name="tr" as="element()" select="local:element('tr')"/>
                <xsl:for-each select="1 to $cols">
            ...
            <xsl:variable name="tbody" as="element()" select="local:element('tbody')"/>
    ...
    <xsl:template match="button[contains-token(@class, 'table-op')]" mode="ixsl:onclick">
        ...
        <xsl:for-each select="local:current-cell()[not(local:has-spans(local:table-of(.)))]">
            <xsl:choose>
                <xsl:when test="$op = ('row-above', 'row-below')">
                    <xsl:call-template name="local:op-insert-row">
                        <xsl:with-param name="cell" select="."/>
                        <xsl:with-param name="before" select="$op = 'row-above'"/>
                    </xsl:call-template>
                </xsl:when>
                <xsl:when test="$op = ('col-left', 'col-right')">
                    <xsl:call-template name="local:op-insert-column">
                        <xsl:with-param name="cell" select="."/>
                        <xsl:with-param name="before" select="$op = 'col-left'"/>
                    </xsl:call-template>
                </xsl:when>
                <xsl:when test="$op = 'del-row'">
                    <xsl:call-template name="local:op-delete-row">
                        <xsl:with-param name="cell" select="."/>
                    </xsl:call-template>
                </xsl:when>
                <xsl:when test="$op = 'del-col'">
                    <xsl:call-template name="local:op-delete-column">
                        <xsl:with-param name="cell" select="."/>
                    </xsl:call-template>
                </xsl:when>
                <xsl:otherwise/>
    ...
    <xsl:template name="local:op-insert-row">
        <xsl:param name="cell" as="element()"/>
        <xsl:param name="before" as="xs:boolean"/>

        <xsl:variable name="i" as="xs:integer" select="local:column-index($cell)"/>
        <xsl:variable name="snapshot-root" as="element()?" select="local:root-of($cell)"/>
        <xsl:variable name="snapshot" as="xs:string?" select="$snapshot-root ! string(ixsl:get(., 'innerHTML'))"/>
        <xsl:variable name="new" as="element()?">
            <xsl:call-template name="local:insert-row">
    ...
    <xsl:template name="local:op-insert-column">
        <xsl:param name="cell" as="element()"/>
        <xsl:param name="before" as="xs:boolean"/>
        ...
        <xsl:for-each select="local:table-rows(local:table-of($cell))">
    ...
    <xsl:template name="local:op-delete-row">
        <xsl:param name="cell" as="element()"/>

        <xsl:variable name="row" as="element()" select="$cell/parent::tr"/>
        <xsl:variable name="table" as="element()?" select="local:table-of($cell)"/>
        <xsl:variable name="body-rows" as="element()*" select="local:table-rows($table)[not(parent::thead)]"/>
        <xsl:if test="if ($row/parent::thead) then exists($body-rows) else count($body-rows) gt 1">
            <xsl:variable name="i" as="xs:integer" select="local:column-index($cell)"/>
            <xsl:variable name="rows" as="element()*" select="local:table-rows($table)"/>
    ...
    <xsl:template name="local:op-delete-column">
        <xsl:param name="cell" as="element()"/>
        ...
        <xsl:if test="count($row/(td | th)) gt 1">
            ...
            <xsl:variable name="victims" as="element()*"
                select="local:table-rows(local:table-of($cell)) ! (./(td | th))[$i]"/>
            <xsl:call-template name="local:push-undo">
                <xsl:with-param name="host" select="$cell"/>
            </xsl:call-template>
    ...
    <xsl:template name="local:table-tab">
        ...
        <xsl:variable name="table" as="element()?" select="local:table-of($host)"/>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="snapshot" as="xs:string?" select="$snapshot-root ! string(ixsl:get(., 'innerHTML'))"/>
                <xsl:variable name="new" as="element()?">
                    <xsl:for-each select="(local:table-rows($table))[last()]">
                        <xsl:call-template name="local:insert-row">
    ...
    <!-- Enter steps down the column, growing the table below the last row; the
         caller (local:handle-enter) has already pushed undo and cut a selection -->
    <xsl:template name="local:table-enter">
        <xsl:param name="host" as="element()"/>

        <xsl:variable name="table" as="element()?" select="local:table-of($host)"/>
        <xsl:variable name="row" as="element()" select="$host/parent::tr"/>
        <xsl:variable name="i" as="xs:integer" select="local:column-index($host)"/>
    ...
    <!-- riding local:update-breadcrumb, the single caret-awareness choke point -->
    <xsl:template name="local:sync-table-toolbar">
        <xsl:variable name="enabled" as="xs:boolean"
            select="exists(local:current-cell()[not(local:has-spans(local:table-of(.)))])"/>
