...
>
    ...
    <xsl:template match="div[contains-token(@class, 'resource-content')][contains-token(@class, 'row-fluid')]//button[contains-token(@class, 'btn-edit')]" mode="ixsl:onclick">
        ...
        <xsl:variable name="request-uri" select="ldh:href($ldt:base, ldh:absolute-path(base-uri()), map{}, $content-value)"/>
    ...
    <xsl:template match="div[contains-token(@class, 'xhtml-content')][contains-token(@class, 'row-fluid')]//button[contains-token(@class, 'btn-save')]" mode="ixsl:onclick">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$container/@about">
                ...
                <xsl:variable name="update-string" select="replace($content-update-string, '$this', '&lt;' || ldh:absolute-path(base-uri()) || '&gt;', 'q')" as="xs:string"/>
                ...
                <xsl:variable name="request-uri" select="ldh:href($ldt:base, ldh:absolute-path(base-uri()), map{}, ldh:absolute-path(base-uri()))" as="xs:anyURI"/>
                <xsl:variable name="request" as="item()*">
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="content-uri" select="xs:anyURI(ldh:absolute-path(base-uri()) || '#' || $content-id)" as="xs:anyURI"/> <!-- build content URI -->
                ...
                <xsl:variable name="update-string" select="replace($content-append-string, '$this', '&lt;' || ldh:absolute-path(base-uri()) || '&gt;', 'q')" as="xs:string"/>
                <xsl:variable name="update-string" select="replace($update-string, '$content', '&lt;' || $content-uri || '&gt;', 'q')" as="xs:string"/>
                        ...
                        <xsl:variable name="request" as="item()*">
                            <ixsl:schedule-action http-request="map{ 'method': 'PATCH', 'href': $request-uri, 'media-type': 'application/sparql-update', 'body': $update-string }">
                                <xsl:call-template name="onResourceContentUpdate">
                                    <xsl:with-param name="container" select="$container"/>
                                    <xsl:with-param name="uri" select="ldh:absolute-path(base-uri())"/>
                                    <xsl:with-param name="content-value" select="$content-value"/>
                                    <xsl:with-param name="mode" select="$mode"/>
                                </xsl:call-template>
                            </ixsl:schedule-action>
        ...
        <xsl:if test="ixsl:call(ixsl:window(), 'confirm', [ ac:label(key('resources', 'are-you-sure', document(resolve-uri('static/com/atomgraph/linkeddatahub/xsl/bootstrap/2.3.2/translations.rdf', $ac:contextUri)))) ])">
            <xsl:choose>
                ...
                <xsl:when test="$container/@about">
                    ...
                    <xsl:variable name="content-uri" select="$container/@about" as="xs:anyURI"/>
                    <xsl:variable name="update-string" select="replace($content-delete-string, '$this', '&lt;' || ldh:absolute-path(base-uri()) || '&gt;', 'q')" as="xs:string"/>
                    <xsl:variable name="update-string" select="replace($update-string, '$content', '&lt;' || $content-uri || '&gt;', 'q')" as="xs:string"/>
                    <xsl:variable name="request-uri" select="ldh:href($ldt:base, ldh:absolute-path(base-uri()), map{}, ldh:absolute-path(base-uri()))" as="xs:anyURI"/>
                    <xsl:variable name="request" as="item()*">
                        <ixsl:schedule-action http-request="map{ 'method': 'PATCH', 'href': $request-uri, 'media-type': 'application/sparql-update', 'body': $update-string }">
    ...
    <xsl:template match="div[ac:mode(base-uri()) = '&ldh;ContentMode'][contains-token(@class, 'content')][contains-token(@class, 'row-fluid')][acl:mode() = '&acl;Write']" mode="ixsl:ondrop">
        ...
        <xsl:if test="not($content-uri = $drop-content-uri)">
            ...
            <xsl:variable name="update-string" select="replace($content-swap-string, '$this', '&lt;' || ldh:absolute-path(base-uri()) || '&gt;', 'q')" as="xs:string"/>
    ...
    <xsl:template name="ldh:LoadContent">
        ...
        <xsl:variable name="request-uri" select="ldh:href($ldt:base, ldh:absolute-path(base-uri()), map{}, $content-value)" as="xs:anyURI"/>
        <xsl:variable name="request" as="item()*">
