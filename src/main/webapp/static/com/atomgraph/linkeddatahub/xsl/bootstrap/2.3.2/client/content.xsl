...
>
    ...
    <xsl:template match="div[contains-token(@class, 'resource-content')][contains-token(@class, 'row-fluid')]//button[contains-token(@class, 'btn-edit')]" mode="ixsl:onclick">
        ...
        <xsl:variable name="request-uri" select="ldh:href($ldt:base, ac:absolute-path(base-uri()), map{}, $content-value)"/>
    ...
    <!-- save query-content query onclick -->
    
    <xsl:template match="div[contains-token(@class, 'query-content')][contains-token(@class, 'row-fluid')]//button[contains-token(@class, 'btn-save-query')]" mode="ixsl:onclick">
...
<!--    <xsl:template match="div[contains-token(@class, 'query-content')][contains-token(@class, 'row-fluid')]//button[contains-token(@class, 'btn-save-chart')]" mode="ixsl:onclick">
        ...
        <xsl:if test="ixsl:call(ixsl:window(), 'confirm', [ ac:label(key('resources', 'are-you-sure', document(resolve-uri('static/com/atomgraph/linkeddatahub/xsl/bootstrap/2.3.2/translations.rdf', $ac:contextUri)))) ])">
            <xsl:choose>
                ...
                <xsl:when test="$container/@about">
                    ...
                    <xsl:variable name="update-string" select="replace($content-delete-string, '$this', '&lt;' || ac:absolute-path(base-uri()) || '&gt;', 'q')" as="xs:string"/>
                    <xsl:variable name="update-string" select="replace($update-string, '$content', '&lt;' || $content-uri || '&gt;', 'q')" as="xs:string"/>
                    <xsl:variable name="request-uri" select="ldh:href($ldt:base, ac:absolute-path(base-uri()), map{}, ac:absolute-path(base-uri()))" as="xs:anyURI"/>
                    <xsl:variable name="request" as="item()*">
                        <ixsl:schedule-action http-request="map{ 'method': 'PATCH', 'href': $request-uri, 'media-type': 'application/sparql-update', 'body': $update-string }">
    ...
    <xsl:template match="div[contains-token(@class, 'resource-content') or contains-token(@class, 'query-content')]//button[contains-token(@class, 'btn-cancel')]" mode="ixsl:onclick">
        <xsl:variable name="container" select="ancestor::div[contains-token(@class, 'content')]" as="element()"/>
    ...
    <xsl:template name="bs2:QueryForm">
        ...
        <xsl:param name="query" as="xs:string"><![CDATA[SELECT DISTINCT *
WHERE
{
    GRAPH ?g
    { ?s ?p ?o }
...
LIMIT 100]]></sp:text>
        ...
        <!-- add @id and .content.query-content to div.row-fluid -->
        <xsl:for-each select="$container">
            <xsl:variable name="content-id" select="'id' || ac:uuid()" as="xs:string"/>
            <ixsl:set-attribute name="id" select="$content-id"/>
...
<!--                        <dct:title><xsl:value-of select="$title-input/ixsl:get(., 'value')"/></dct:title>-->
        ...
        <xsl:message>CONTENT.XSL $container-uri: <xsl:value-of select="$container-uri"/></xsl:message>
    ...
    <xsl:template match="div[ixsl:query-params()?mode = '&ldh;ContentMode'][contains-token(@class, 'content')][contains-token(@class, 'row-fluid')][acl:mode() = '&acl;Write']" mode="ixsl:ondrop">
        ...
        <xsl:if test="not($content-uri = $drop-content-uri)">
            ...
            <xsl:variable name="update-string" select="replace($content-swap-string, '$this', '&lt;' || ac:absolute-path(base-uri()) || '&gt;', 'q')" as="xs:string"/>
