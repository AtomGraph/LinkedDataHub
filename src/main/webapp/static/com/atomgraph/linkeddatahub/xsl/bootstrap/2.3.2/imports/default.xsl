...
>
    ...
    <xsl:template match="@rdf:about" mode="xhtml:Anchor">
        ...
        <xsl:param name="id" select="if (starts-with(., '#') or (ac:absolute-path(.) = ac:absolute-path(ldh:base-uri(.)) and (contains(., '#')))) then substring-after(., '#') else (if (starts-with(., $ldt:base)) then () else encode-for-uri(.))" as="xs:string?"/>
    ...
    <xsl:template match="@rdf:resource" mode="bs2:FormControl">
        ...
        <xsl:message><xsl:value-of select="."/> $forClass: <xsl:value-of select="$forClass"/></xsl:message>
