...
WHERE
  {   { ?classOrShape (rdfs:subClassOf)*/spin:constructor ?constructor
    ...
    <xsl:template match="form[contains-token(@class, 'form-horizontal')] | form[ancestor::div[contains-token(@class, 'modal')]]" mode="ixsl:onsubmit">
        ...
        <xsl:variable name="request-uri" select="ldh:href($ldt:base, ldh:absolute-path(base-uri()), map{}, $action)" as="xs:anyURI"/>
    ...
    <xsl:template match="button[contains-token(@class, 'add-value')]" mode="ixsl:onclick">
        ...
        <xsl:variable name="href" select="ac:build-uri(ldh:absolute-path(base-uri()), map{ 'forClass': for $class in $forClass return string($class) })" as="xs:anyURI"/>
    ...
    <xsl:template match="input[contains-token(@class, 'typeahead')]" mode="ixsl:onkeyup">
        ...
        <xsl:variable name="request-uri" select="ldh:href($ldt:base, ldh:absolute-path(base-uri()), map{}, $results-uri)" as="xs:anyURI"/> <!-- proxy the results -->
    ...
    <xsl:template match="ul[contains-token(@class, 'dropdown-menu')][contains-token(@class, 'type-typeahead')]/li" mode="ixsl:onmousedown" priority="1">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$resource/rdf:type/@rdf:resource = '&sh;NodeShape'">
                ...
                <xsl:variable name="href" select="ac:build-uri(ldh:absolute-path(base-uri()), map{ 'forShape': string($forShape) })" as="xs:anyURI"/>
                <xsl:variable name="request" as="item()*">
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="href" select="ac:build-uri(ldh:absolute-path(base-uri()), map{ 'forClass': string($forClass) })" as="xs:anyURI"/>
                <xsl:variable name="request" as="item()*">
                    <!-- use Control-Cache: no-cache to get fresh HTML -->
                    <ixsl:schedule-action http-request="map{ 'method': 'GET', 'href': $href, 'headers': map{ 'Accept': 'application/xhtml+xml', 'Cache-Control': 'no-cache' } }">
                        <xsl:call-template name="onAddConstructor">
