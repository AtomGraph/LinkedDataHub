...
<!--        <xsl:sequence select="ac:label(.)"/>-->
    ...
    <xsl:template match="*[*][@rdf:about] | *[*][@rdf:nodeID]" mode="bs2:FormControl">
        ...
        <xsl:param name="template-doc" select="ac:construct-doc($apl:ontology, $forClass, $apl:base)" as="document-node()?"/>
        ...
        <fieldset>
            ...
            <xsl:apply-templates select="* | $template/*[not(concat(namespace-uri(), local-name()) = current()/*/concat(namespace-uri(), local-name()))][not(self::rdf:type)][not(self::foaf:isPrimaryTopicOf)]" mode="#current">
                ...
                <xsl:sort select="key('resources', key('resources', ../rdf:type/@rdf:resource, document(ac:document-uri(../rdf:type/@rdf:resource)))/spin:constraint/(@rdf:resource|@rdf:nodeID))[rdf:type/@rdf:resource = '&apl;MissingPropertyValue'][sp:arg1/@rdf:resource = current()/concat(namespace-uri(), local-name())]" order="descending"/>
