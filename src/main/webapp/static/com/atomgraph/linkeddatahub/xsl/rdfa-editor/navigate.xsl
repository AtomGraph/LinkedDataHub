...
-->
    ...
    <xsl:function name="local:rank" as="xs:integer">
        <xsl:param name="heading" as="element()"/>
        <xsl:sequence select="xs:integer(substring(local-name($heading), 2))"/>
    </xsl:function>

    <xsl:template name="local:render-toc">
        <xsl:variable name="root" as="element()?" select="local:active-root()"/>
            ...
            <xsl:result-document href="?." method="ixsl:replace-content">
                <xsl:choose>
                    <xsl:when test="exists($headings)">
                        <xsl:call-template name="local:toc-level">
    ...
    <xsl:template match="li[contains-token(@class, 'toc-item')]" mode="ixsl:ondrop">
        ...
        <xsl:if test="exists($source) and local:has-transfer-type($event, 'application/x-rdfa-editor-section')">
            ...
            <xsl:if test="exists($target) and not($target is $source)
                ...
                <xsl:variable name="section" as="element()+" select="local:section-of($source)"/>
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:iterate select="$section">
                            <xsl:param name="anchor" as="element()" select="local:section-of($target)[last()]"/>
    ...
    <xsl:function name="local:compact-term" as="xs:string">
        <xsl:param name="term" as="xs:string"/>
        <xsl:sequence select="if (matches($term, '^[a-zA-Z][a-zA-Z0-9+.-]*:'))
            then rdfa:prefixed-name(rdfa:split-uri($term), $rdfa:default-prefixes)
            else $term"/>
    </xsl:function>

    <xsl:function name="local:crumb-label" as="xs:string">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:sequence select="local-name($element) || ($term ! ('[' || local:compact-term(.) || ']'), '')[1]"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
                    ...
                    <xsl:result-document href="?." method="ixsl:replace-content">
                        <xsl:for-each select="$ancestors">
                            ...
                            <span class="crumb" data-index="{position()}">
                                <xsl:value-of select="local:crumb-label(.)"/>
                            </span>
                        </xsl:for-each>
                    </xsl:result-document>
                </xsl:for-each>
    ...
    <xsl:template name="local:run-lint">
    ...
    <xsl:function name="local:norm" as="xs:string">
    ...
    <xsl:function name="local:find-in" as="xs:integer?">
        <xsl:param name="hay" as="xs:string"/>
        <xsl:param name="from" as="xs:integer"/>
        <xsl:param name="query" as="xs:string"/>
        <xsl:param name="ci" as="xs:boolean"/>
        <xsl:variable name="tail" as="xs:string" select="local:norm(substring($hay, $from), $ci)"/>
        <xsl:variable name="q" as="xs:string" select="local:norm($query, $ci)"/>
    ...
    <xsl:function name="local:replace-in-string" as="map(*)">
        <xsl:param name="hay" as="xs:string"/>
        <xsl:param name="query" as="xs:string"/>
        <xsl:param name="replacement" as="xs:string"/>
        <xsl:param name="ci" as="xs:boolean"/>
        <xsl:variable name="p" as="xs:integer?" select="local:find-in($hay, 1, $query, $ci)"/>
        <xsl:choose>
            <xsl:when test="empty($p)">
                <xsl:sequence select="map{ 'value': $hay, 'count': 0 }"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:variable name="rest" as="map(*)" select="local:replace-in-string(
    ...
    <xsl:template name="local:find-status">
    ...
    <xsl:template name="local:find-next">
        <xsl:variable name="dialog" as="element()" select="id('find-dialog', ixsl:page())"/>
            ...
            <xsl:otherwise>
                ...
                <xsl:iterate select="$plan">
                    ...
                    <xsl:variable name="position" as="xs:integer?"
                        select="local:find-in(string($node), ?from, $query, $ci)"/>
                    <xsl:choose>
    ...
    <xsl:template match="button[contains-token(@class, 'replace-all')]" mode="ixsl:onclick">
        ...
        <xsl:variable name="ci" as="xs:boolean"
            select="not(ixsl:get(($dialog//input[@name = 'match-case'])[1], 'checked'))"/>
        <xsl:variable name="matched" select="local:roots()//text()[not(ancestor::*[@data-role])]
            [$query ne ''][contains(local:norm(string(.), $ci), local:norm($query, $ci))]"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:iterate select="$matched">
                    ...
                    <xsl:variable name="result" as="map(*)"
                        select="local:replace-in-string(string(.), $query, $replacement, $ci)"/>
