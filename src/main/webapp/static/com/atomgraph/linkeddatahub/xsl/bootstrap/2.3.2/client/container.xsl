...
<!--                                    <xsl:if test="$filter/*/@rdf:resource = @rdf:about">
    ...
    <xsl:template name="render-container">
        ...
        <xsl:if test="$active-mode = '&ac;MapMode'">
            ...
            <xsl:variable name="initial-load" select="not(ixsl:contains(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'map'))" as="xs:boolean"/>
            <!-- reuse center and zoom if map object already exists, otherwise set defaults -->
            <xsl:variable name="center-lat" select="if (not($initial-load)) then xs:float(ixsl:call(ixsl:call(ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'map'), 'getCenter', []), 'lat', [])) else 56" as="xs:float"/>
            <xsl:variable name="center-lng" select="if (not($initial-load)) then xs:float(ixsl:call(ixsl:call(ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'map'), 'getCenter', []), 'lng', [])) else 10" as="xs:float"/>
            <xsl:variable name="zoom" select="if (not($initial-load)) then xs:integer(ixsl:call(ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'map'), 'getZoom', [])) else 4" as="xs:integer"/>
            
            <ixsl:set-property name="map" select="ac:create-map($canvas-id, $center-lat, $center-lng, $zoom)" object="ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value)"/>
            ...
            <ixsl:set-property name="geo" select="ac:create-geo-object($content-value, ac:uri(), $ldt:base, $endpoint, $select-string, $focus-var-name, $graph-var-name)" object="ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value)"/>
    ...
    <xsl:template match="*[contains-token(@class, 'resource-content')]//div/ul[@class = 'nav nav-tabs']/li[not(contains-token(@class, 'active'))]/a" mode="ixsl:onclick">
        ...
        <xsl:variable name="content-value" select="xs:anyURI(translate(ixsl:get($container, 'dataset.contentValue'), '.', '-'))" as="xs:anyURI"/>
        <xsl:variable name="content" select="ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'content')" as="element()"/>
        <xsl:variable name="active-class" select="../@class" as="xs:string"/>
        <xsl:variable name="select-xml" select="ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'select-xml')" as="document-node()"/>
        <xsl:variable name="focus-var-name" select="ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'focus-var-name')" as="xs:string"/>
        <xsl:variable name="service-uri" select="if (ixsl:contains(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'service-uri')) then ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'service-uri') else ()" as="xs:anyURI?"/>
        ...
        <xsl:call-template name="render-container">
            ...
            <xsl:with-param name="content-value" select="$content-value"/>
            <xsl:with-param name="content" select="$content"/>
            <xsl:with-param name="results" select="ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'results')"/>
            <xsl:with-param name="select-xml" select="$select-xml"/>
            <xsl:with-param name="focus-var-name" select="$focus-var-name"/>
    ...
    <xsl:template match="*[contains-token(@class, 'resource-content')]//ul[@class = 'pager']/li[@class = 'previous']/a[@class = 'active']" mode="ixsl:onclick">
        ...
        <xsl:variable name="select-string" select="ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'select-query')" as="xs:string"/>
        <xsl:variable name="select-xml" select="ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value), 'select-xml')" as="document-node()"/>
        <xsl:variable name="offset" select="if ($select-xml/json:map/json:number[@key = 'offset']) then xs:integer($select-xml/json:map/json:number[@key = 'offset']) else 0" as="xs:integer"/>
        ...
        <xsl:choose>
            <xsl:when test="?status = 200 and ?media-type = 'application/rdf+xml'">
                <xsl:for-each select="?body">
                    ...
                    <ixsl:set-property name="results" select="$grouped-results" object="ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub.contents'), $content-value)"/>
