...
>
    ...
    <!--
        The platform's shared library: keys, global params, functions, the design-system component
        emitters (ldh:Modal, ldh:DataTable, ldh:PropertyLabel, ldh:BlockBlank, ldh:BlockBar,
        ldh:ProgressBar, ldh:FormFooter, ac:AnnotationTag), the ldh:Combobox widget and the RDF/XML
        rewriters (ldh:RenameBnode, ldh:Reserialize, ldh:SetResourceID, ldh:SetPrimaryTopic,
        ldh:XHTMLContent). Imported through common.xsl, ABOVE the package stylesheets, so none of it can
        be redefined by a package.

        The generic value rendering - property rows, value cells, form controls, annotations - used to
        live here too and now lives in imports/values.xsl, the open tier under the packages, precisely so
        that a vocabulary package can specialise it. A rule belongs here only if it is typed on core
        vocabulary (the dh: document slug control) or is not a value at all.
    -->
    ...
    <!-- Builds the pure, bnode-prototyped constructor consumed by ac:FormControl from the two raw inputs the promise chain has fetched: $shapes (SHACL NodeShape RDF) and $constructed-doc (SPIN constructor RDF fetched via the ns SPARQL endpoint). Shared by every flow that ends in ac:FormControl (ldh:render-row-form for EDIT, ldh:render-row-form-violation for violation re-render, and the Phase 4 modal/app-settings/signup renderers to come) so the merge logic lives in exactly one place. Returns the SPIN side unchanged when shapes are absent (typical for system classes like sp:Describe), returns the merged doc when both sides exist (user-defined classes like skos:Concept with both SPIN defaults and SHACL constraints), or empty if neither side provides input. -->
    ...
    <xsl:template match="*[rdf:type/@rdf:resource = ('&dh;Container', '&dh;Item')]/@rdf:about | *[rdf:type/@rdf:resource = ('&dh;Container', '&dh;Item')]/@rdf:nodeID" mode="ac:FormControl" priority="1">
        <xsl:param name="type" select="'text'" as="xs:string"/>
        <xsl:param name="id" select="generate-id()" as="xs:string"/>
        ...
        <div class="ldh-subject">
            ...
            <span class="ac-select-locked sz-sm">
                ...
                <select class="subject-type" disabled="disabled" tabindex="-1">
                    <option value="su" selected="selected"><xsl:apply-templates select="key('resources', 'term-uri', ldh:translations())" mode="ac:label"/></option>
                </select>
            </span>
