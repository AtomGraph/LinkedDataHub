...
<!--                    <xsl:if test="not($query)">
    ...
    <xsl:template match="/" mode="ldh:HTMLDocumentLoaded">
        <xsl:param name="href" select="base-uri()" as="xs:anyURI"/> <!-- possibly proxied URL -->
        <xsl:param name="container" as="element()"/>
        <xsl:param name="push-state" select="true()" as="xs:boolean"/>
        <xsl:param name="endpoint" as="xs:anyURI?"/>
        ...
        <xsl:message>
            ...
            id('content-body', ixsl:page())/div[contains-token(@class, 'row-fluid')][1]/ul[contains-token(@class, 'nav-tabs')]/li[contains-token(@class, 'content-mode')][contains-token(@class, 'active')]: <xsl:value-of select="id('content-body', ixsl:page())/div[contains-token(@class, 'row-fluid')][1]/ul[contains-token(@class, 'nav-tabs')]/li[contains-token(@class, 'content-mode')][contains-token(@class, 'active')]"/>
            id('content-body', ixsl:page())/div[contains-token(@class, 'row-fluid')][1]/ul[contains-token(@class, 'nav-tabs')]/li[contains-token(@class, 'content-mode')]: <xsl:value-of select="id('content-body', ixsl:page())/div[contains-token(@class, 'row-fluid')][1]/ul[contains-token(@class, 'nav-tabs')]/li[contains-token(@class, 'content-mode')]"/>
        </xsl:message>
    ...
    <xsl:template match="a[not(@target)][starts-with(resolve-uri(@href, base-uri()), 'http://') or starts-with(resolve-uri(@href, base-uri()), 'https://')][not(starts-with(resolve-uri(@href, base-uri()), resolve-uri('uploads/', $ldt:base)))][ancestor::div[@id = 'breadcrumb-nav'] or not(ancestor::div[tokenize(@class, ' ') = ('navbar', 'footer')])] | a[contains-token(@class, 'brand')] | div[button[contains-token(@class, 'btn-apps')]]/ul//a | svg:a[not(@target)][starts-with(resolve-uri(@href, base-uri()), 'http://') or starts-with(resolve-uri(@href, base-uri()), 'https://')][not(starts-with(resolve-uri(@href, base-uri()), resolve-uri('uploads/', $ldt:base)))]" mode="ixsl:onclick">
        <xsl:sequence select="ixsl:call(ixsl:event(), 'preventDefault', [])"/>
        <xsl:variable name="href" select="resolve-uri(@href, base-uri())" as="xs:anyURI"/> <!-- resolve relative URIs -->
    ...
    <xsl:template match="form[contains-token(@class, 'navbar-form')]" mode="ixsl:onsubmit">
        ...
        <xsl:if test="$uri-string castable as xs:anyURI and (starts-with($uri-string, 'http://') or starts-with($uri-string, 'https://'))">
            ...
            <xsl:variable name="href" select="ldh:href($ldt:base, ac:absolute-path(base-uri()), map{}, $uri)" as="xs:anyURI"/>
            <ixsl:set-style name="cursor" select="'progress'" object="ixsl:page()//body"/>
    ...
    <xsl:template name="onDelete">
        ...
        <xsl:choose>
            <xsl:when test="?status = 204"> <!-- No Content -->
                <xsl:variable name="href" select="resolve-uri('..', base-uri())" as="xs:anyURI"/>
                <xsl:variable name="request-uri" select="ldh:href($ldt:base, ac:absolute-path(base-uri()), map{}, $href)" as="xs:anyURI"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$key-code = 'Enter'">
                <xsl:if test="$menu/li[contains-token(@class, 'active')]">
                    ...
                    <xsl:variable name="request-uri" select="ldh:href($ldt:base, ac:absolute-path(base-uri()), map{}, $uri)" as="xs:anyURI"/>
