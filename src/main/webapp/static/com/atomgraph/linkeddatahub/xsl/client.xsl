            ...
            <xsl:when test="?status = 200 and ?media-type = 'application/xhtml+xml'">
                <xsl:for-each select="?body">
                    ...
                    <xsl:for-each select="id('export-rdf', ixsl:page())/following-sibling::ul/li/a">
                        ...
                        <xsl:variable name="href" select="ac:build-uri($local-uri, let $params := map{ 'accept': @title } return if (not(starts-with($uri, $ldt:base))) then map:merge(($params, map{ 'uri': $uri})) else $params)" as="xs:anyURI"/>
