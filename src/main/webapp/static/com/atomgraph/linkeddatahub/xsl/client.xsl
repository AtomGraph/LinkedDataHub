...
>
    ...
    <xsl:param name="a:graphStore" select="resolve-uri('service', $apl:base)" as="xs:anyURI?"/>
                            ...
                            <div class="tabbable">
                                ...
                                <div class="tab-content">
                                    ...
                                    <div>
                                        ...
                                        <form id="form-clone-data" method="POST" action="{resolve-uri('clone', $apl:base)}">
                                            <xsl:comment>This form uses RDF/POST encoding: http://www.lsrn.org/semweb/rdfpost.html</xsl:comment>
                                            <xsl:call-template name="xhtml:Input">
                                                <xsl:with-param name="name" select="'rdf'"/>
                                                <xsl:with-param name="type" select="'hidden'"/>
                    ...
                    <xsl:variable name="results" select="." as="document-node()"/>
                        ...
                        <xsl:result-document href="#{$id}" method="ixsl:replace-content">
                            <xsl:if test="$container-list">
                                <div class="well well-small">
                                    <h2 class="nav-header">
                                        <a href="{$apl:base}" title="{$apl:base}">
                                            <xsl:value-of>
...
<!--        <xsl:message>
    ...
    <!-- intercept all link HTTP(S) clicks except to /uploads/ and those in the navbar (except breadcrumb bar) and the footer -->
    <xsl:template match="a[not(@target)][starts-with(@href, 'http://') or starts-with(@href, 'https://')][not(starts-with(@href, resolve-uri('uploads/', $apl:base)))][ancestor::div[@id = 'breadcrumb-nav'] or not(ancestor::div[tokenize(@class, ' ') = ('navbar', 'footer')])]" mode="ixsl:onclick">
...
<!--                <xsl:with-param name="container-id" select="$container-id"/>-->
    ...
    <xsl:template match="div[tokenize(@class, ' ') = 'hero-unit']/button[tokenize(@class, ' ') = 'close']" mode="ixsl:onclick" priority="1">
        ...
        <ixsl:set-property name="cookie" select="concat('LinkedDataHub.first-time-message=true; path=/', substring-after($apl:base, $ac:contextUri), '; expires=Fri, 31 Dec 9999 23:59:59 GMT')" object="ixsl:page()"/>
    </xsl:template>
    ...
    <xsl:template match="select[tokenize(@class, ' ') = 'chart-type']" mode="ixsl:onchange">
        ...
        <xsl:param name="content-uri" select="translate((ixsl:get($container, 'dataset.contentUri'), ancestor::div[contains-token(@class, 'sparql-results')]/preceding-sibling::input[@name = 'href']/@value)[1], '.', '-')" as="xs:anyURI"/>        <xsl:param name="chart-canvas-id" select="ancestor::form/following-sibling::div/@id" as="xs:string"/>
    ...
    <xsl:template match="select[tokenize(@class, ' ') = 'chart-category']" mode="ixsl:onchange">
        ...
        <!-- $content-uri value comes either from resource-content's @data-content-uri or from an input in the 'query-form' -->
        <xsl:variable name="container" select="ancestor::div[tokenize(@class, ' ') = 'resource-content']" as="element()"/>
        <xsl:param name="content-uri" select="translate((ixsl:get($container, 'dataset.contentUri'), ancestor::div[contains-token(@class, 'sparql-results')]/preceding-sibling::input[@name = 'href']/@value)[1], '.', '-')" as="xs:anyURI"/>
    ...
    <xsl:template match="select[tokenize(@class, ' ') = 'chart-series']" mode="ixsl:onchange">
        ...
        <!-- $content-uri value comes either from resource-content's @data-content-uri or from an input in the 'query-form' -->
        <xsl:variable name="container" select="ancestor::div[tokenize(@class, ' ') = 'resource-content']" as="element()"/>
        <xsl:param name="content-uri" select="xs:anyURI(translate((ixsl:get($container, 'dataset.contentUri'), ancestor::div[contains-token(@class, 'sparql-results')]/preceding-sibling::input[@name = 'href']/@value)[1], '.', '-'))" as="xs:anyURI"/>
        <xsl:param name="chart-canvas-id" select="ancestor::form/following-sibling::div/@id" as="xs:string"/>
        <xsl:variable name="results" select="ixsl:get(ixsl:get(ixsl:get(ixsl:window(), 'LinkedDataHub'), $content-uri), 'results')" as="document-node()"/>
