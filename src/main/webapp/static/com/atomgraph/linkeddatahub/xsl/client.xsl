...
<!--        <ixsl:schedule-action http-request="map{ 'method': 'GET', 'href': $apl:ontology, 'headers': map{ 'Accept': 'application/rdf+xml' } }">
    ...
    <!-- chart content -->
    <xsl:template match="*[@rdf:about][spin:query/@rdf:resource][apl:chartType/@rdf:resource]" mode="apl:Content" priority="1">
        <xsl:param name="container" as="element()"/>
        <!-- replace dots with dashes to avoid Saxon-JS treating them as field separators: https://saxonica.plan.io/issues/5031 -->
        <xsl:param name="content-uri" select="xs:anyURI(translate(@rdf:about, '.', '-'))" as="xs:anyURI"/>
...
<!--        <xsl:message>
    ...
    <xsl:template match="select[tokenize(@class, ' ') = 'chart-type']" mode="ixsl:onchange">
        ...
        <xsl:variable name="content-uri" select="translate((if ($container) then ixsl:get($container, 'dataset.contentUri') else (), ancestor::div[contains-token(@class, 'sparql-results')]/preceding-sibling::input[@name = 'href']/@value)[1], '.', '-')" as="xs:anyURI"/>
